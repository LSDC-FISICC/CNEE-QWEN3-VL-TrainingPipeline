...
Usa dataset_inferencia_test.json (sin respuesta del asistente).
...
print("=== Inferencia Qwen3-VL — Casos nuevos (no vistos en training) ===\n")
...
print(f"\nInferencia sobre {n_total} casos. ETA estimado: {n_total * 15 / 60:.0f} minutos\n")
...
fig.suptitle(f"Inferencia — Casos nuevos (n={n_total}) — Qwen3-VL 4B (CNEE)",
...
# Plot 3: Confusion matrix sistema (sin escalados)
ax = axes[1, 0]
...
# Plot 4: Indicadores globales modelo vs sistema
ax = axes[1, 1]
indicadores = ['VQA\nAccuracy', 'Auto\nSegura', 'Falsos+\nAuto', 'Escalacion']
vals_modelo  = [m['vqa_pct'], m['tasa_auto_segura'], m['tasa_falsos_auto'], m['tasa_escalacion']]
cols_bar = ['#16A34A', '#16A34A', '#DC2626', '#F59E0B']
bars = ax.bar(indicadores, vals_modelo, color=cols_bar, width=0.5)
ax.set_title("Indicadores Globales — Casos Nuevos")
ax.set_ylabel("Porcentaje (%)")
ax.set_ylim(0, 110)
ax.grid(True, alpha=0.3, axis="y")
for bar, v in zip(bars, vals_modelo):
